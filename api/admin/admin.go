...
func main() {
	...
	// 停止后台推送必须注册为关闭监听，**不能用 defer**：
	// 进程收到 SIGTERM/SIGINT 时 go-zero 先跑关闭监听、再用 syscall.Kill 把自己
	// 杀掉（core/proc/shutdown.go:96），main 不会返回，所有 defer 都不会执行。
	proc.AddShutdownListener(ctx.Shutdown)
