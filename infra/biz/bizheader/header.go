...
// BearerScheme 是 Authorization 头携带访问令牌的认证方案前缀（RFC 6750 §2.1）
const BearerScheme = "Bearer "

// Token 取出请求头中的访问令牌，剥离可选的认证方案前缀。
//
// RFC 6750 §2.1 规定凭据以 "Bearer <token>" 形式携带（方案名不区分大小写）；
// 同时接受不带方案的裸令牌，以兼容直接携带的调用方。
func Token(header http.Header, name string) string {
	value := strings.TrimSpace(header.Get(name))
	if len(value) >= len(BearerScheme) && strings.EqualFold(value[:len(BearerScheme)], BearerScheme) {
		return strings.TrimSpace(value[len(BearerScheme):])
	}
