...
import (
	...
	"github.com/ve-weiyi/blog-cloud/infra/gormlogx"
	"github.com/ve-weiyi/blog-cloud/infra/storex"
	"github.com/ve-weiyi/blog-cloud/infra/storex/otpx"
...
type ServiceContext struct {
	...
	EmailDeliver emailx.IEmailDeliver
...
func NewServiceContext(c config.Config) *ServiceContext {
	...
	smsProvider, err := NewSmsProvider(c.SmsConf)
	if err != nil {
		panic(err)
	}

	oAuthProviders, err := NewOAuthProviders(c.AppOAuthConf)
	if err != nil {
		panic(err)
	}
...
func NewEmailDeliver(c config.EmailConf) emailx.IEmailDeliver {
	ec := &emailx.EmailConfig{
		Host:     c.Host,
		Port:     c.Port,
		Username: c.Username,
		Password: c.Password,
	...
	return emailx.NewEmailDeliver(ec)
}

func NewSmsProvider(c config.SmsConf) (smsx.SmsProvider, error) {
	sc := &smsx.SmsConfig{
		Provider:  c.Provider,
...
func NewOAuthProviders(confs map[string]config.OAuthConf) (map[string]oauthx.OAuthProvider, error) {
	providers := make(map[string]oauthx.OAuthProvider)
	for platform, c := range confs {
		p, err := oauthx.NewOAuthProvider(&oauthx.OAuthConfig{
			Platform:     platform,
			ClientId:     c.ClientId,
