...
func (l *ListRolesLogic) ListRoles(in *accessrpc.ListRolesRequest) (*accessrpc.ListRolesResponse, error) {
	...
	page := int(in.ListQuery.GetPage())
	size := int(in.ListQuery.GetPageSize())
	...
	if size <= 0 {
		...
		out := &accessrpc.ListRolesResponse{
	...
	sorts := strings.Join(in.ListQuery.GetSorts(), ",")
