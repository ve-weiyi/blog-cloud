...
func (l *GetTalkLogic) GetTalk(in *discussionrpc.GetTalkRequest) (*discussionrpc.GetTalkResponse, error) {
	entity, err := l.svcCtx.TTalkModel.FindById(l.ctx, in.Id)
	if err != nil {
		return nil, err
	}
	return &discussionrpc.GetTalkResponse{Talk: convertTalkOut(entity)}, nil
}
